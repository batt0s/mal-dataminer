...
class Anime:
    ...
    def from_json(cls, json_obj: Dict[str, Any]):
        return cls(
            ...
            picture=json_obj['main_picture']['medium'] if 'main_picture' in json_obj else "",
            title_en=json_obj['alternative_titles']['en'] if 'alternative_titles' in json_obj else "",
            title_ja=json_obj['alternative_titles']['ja'] if 'alternative_titles' in json_obj else "",
            start_date=json_obj['start_date'] if 'start_date' in json_obj else "",
            end_date=json_obj['end_date'] if 'end_date' in json_obj else "",
            synopsis=json_obj['synopsis'] if 'synopsis' in json_obj else "",
            mean_score=json_obj['mean'] if 'mean' in json_obj else "",
            rank=json_obj['rank'] if 'rank' in json_obj else "",
            popularity=json_obj['popularity'] if 'popularity' in json_obj else "",
            nsfw=True if json_obj['nsfw'] != "white" else False,
            media_type=json_obj['media_type'],
            status=json_obj['status'],
            genres=",".join([genre['name'] for genre in json_obj['genres']]),
            ...
            studios=",".join([studio['name'] for studio in json_obj['studios']]),
